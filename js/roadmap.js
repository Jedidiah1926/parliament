        ...
        cards: [
          ...
          },
          {
            ...
            desc: `2026.09.22
              ...
              <br>- 화면 최상단에 크롬 탭 스타일의 세이브 탭 바 신설 — 클릭 한 번으로 확인창 없이 세이브 사이를 즉시 전환
              <br>- 자동저장을 전역 슬롯 하나가 아니라 이름 붙인 세이브마다 전용 자동저장("OO 자동저장")을 따로 갖도록 개편, 세이브 전환/삭제 시에도 서로 진행 상황이 섞이지 않음
              <br>- 특정 세이브에 속하지 않는 기본 자동저장 슬롯 이름을 "자동저장"에서 "기존 저장"으로 변경
              <br>- 메인 화면 세이브 선택 창에 저장 파일(.json) 업로드로 바로 이어하기 기능 추가
              <br>- 세이브 탭 "+"에 "새로 생성"/"프리셋에서 생성" 옵션 추가, presets/ 폴더 + index.json으로 미리 만든 시나리오를 배포·공유할 수 있는 구조 도입
              <br>- 구버전(통합 이전) 자동저장 데이터 자동 이관 처리
